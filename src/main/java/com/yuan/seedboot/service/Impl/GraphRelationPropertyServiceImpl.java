...
import com.yuan.seedboot.mapper.GraphRelationPropertyMapper;
...
/**
 * @description 针对表【graph_relation_property(关系属性)】的数据库操作Service实现
 */
@Slf4j
@Service
public class GraphRelationPropertyServiceImpl extends ServiceImpl<GraphRelationPropertyMapper, GraphRelationProperty>
        implements GraphRelationPropertyService {

    @Override
    public GraphRelationProperty addProperty(RelationPropertyAddRequest request) {
        ThrowUtils.throwIf(request == null, ErrorCode.PARAMS_ERROR);
        ThrowUtils.throwIf(request.getRelationTypeId() == null, ErrorCode.PARAMS_ERROR, "关系类型 id 为空");
        ...
        ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR, "新增关系属性失败");
        return property;
    }
    ...
    @Override
    public GraphRelationProperty updateProperty(RelationPropertyUpdateRequest request) {
        ThrowUtils.throwIf(request == null, ErrorCode.PARAMS_ERROR);
        ThrowUtils.throwIf(request.getId() == null || request.getId() <= 0, ErrorCode.PARAMS_ERROR, "属性 id 为空");
        ThrowUtils.throwIf(StrUtil.isBlank(request.getPropertyName()), ErrorCode.PARAMS_ERROR, "属性名称为空");
        ThrowUtils.throwIf(StrUtil.isBlank(request.getPropertyType()), ErrorCode.PARAMS_ERROR, "属性类型为空");
        GraphRelationProperty property = this.getById(request.getId());
        ThrowUtils.throwIf(property == null, ErrorCode.NOT_FOUND_ERROR, "关系属性不存在");
        ...
        ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR, "更新关系属性失败");
        return property;
    }
}
